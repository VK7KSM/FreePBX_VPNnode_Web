...
window.ElfMedia=(function(){
  ...
  function controls(d){var rows=[['ptt','PTT'],['call','电话'],['microphone','麦克风'],['photo','拍照'],['video','录像'],['alarm','警报']];return rows.map(function(row){var selected=active&&active.mode===row[0],disabled=!d||!ElfMediaCapabilities.allows(d,row[0])||active&&!selected;return '<button type="button" class="'+(selected?'active':'')+'" aria-pressed="'+!!selected+'" onclick="ElfMedia.start(\''+row[0]+'\')"'+(disabled?' disabled':'')+'>'+row[1]+'</button>';}).join('');}
