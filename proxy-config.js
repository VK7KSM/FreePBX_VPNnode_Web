...
export const PROXY_CONFIG_MAX_BYTES=64*1024;
...
const validTaskId=value=>typeof value==='string'&&/^[A-Za-z0-9-]{1,64}$/.test(value);
...
export function proxyConfigureParams(device,input,taskId){
  ...
  if(!meta||!validSha(meta.sha256)||!Number.isInteger(meta.size)||meta.size<1||meta.size>PROXY_CONFIG_MAX_BYTES)throw Error('请先上传代理配置');
  ...
  const query=new URLSearchParams({device_id:device.id,task_id:taskId});
  return {config_version:meta.version,config_sha256:meta.sha256,config_size:meta.size,
    download_url:'/api/elfremote/proxy-config/download?'+query,rollback_required:true,validation_required:true};
}
...
export async function proxyConfigMetadata(storage,request,loadDevices,saveDevices,now=Date.now()){
  try{
    ...
    if(data.action==='authorize'){
      if(!validTaskId(data.task_id)||typeof data.token!=='string'||data.token.length>512)return json({ok:false,msg:'设备凭证无效'},401);
      if(device.token_sha256!==await sha256(new TextEncoder().encode(data.token)))return json({ok:false,msg:'设备凭证无效'},401);
      ...
      if(device.enabled===false||!active(task)||task.id!==data.task_id||task.type!=='configure_proxy'||task.cancel_requested
        ||!meta||task.params?.config_sha256!==meta.sha256||task.params?.config_version!==meta.version||task.params?.config_size!==meta.size)
    ...
    if(data.action==='register'){
      ...
      if(device.enabled===false||device.managed_proxy_tasks!==true)return json({ok:false,msg:'客户端尚未支持代理管理'},409);
      if(active(device.task))return json({ok:false,msg:'已有任务进行中'},409);
      if(!validSha(data.sha256)||!Number.isInteger(data.size)||data.size<1||data.size>PROXY_CONFIG_MAX_BYTES
...
export async function proxyConfigHttp(env,request,stub){
  try{
    ...
    if(url.pathname==='/api/elfremote/proxy-config'&&request.method==='POST'){
      ...
      if(declared!==null&&(!Number.isFinite(declared)||declared<1||declared>PROXY_CONFIG_MAX_BYTES))throw Error('代理配置大小须为1至64 KiB');
      ...
      if(bytes.length<1||bytes.length>PROXY_CONFIG_MAX_BYTES)throw Error('代理配置大小须为1至64 KiB');
    ...
    if(url.pathname==='/api/elfremote/proxy-config/download'&&request.method==='GET'){
      const auth=await rpc(stub,{action:'authorize',device_id:url.searchParams.get('device_id'),task_id:url.searchParams.get('task_id'),
