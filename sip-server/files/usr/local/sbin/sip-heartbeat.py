...
exten => _.,1,NoOp(Internal ${CHANNEL(endpoint)} -> ${EXTEN})
 ...
 same => n,Hangup()
 same => n(fwd),GotoIf($[0${SIPFWDCOUNT} >= 3]?dial)
 same => n,GotoIf($["${DB(CF/${EXTEN})}" = ""]?dial)
 same => n,Set(__SIPFWDCOUNT=$[0${SIPFWDCOUNT} + 1])
...
exten => _.,1,NoOp(Outbound SMS from ${MESSAGE(from)} to ${EXTEN})
 ...
 same => n,Set(PEER=${FILTER(0-9,${EXTEN})})
 same => n,GotoIf($[${LEN(${PEER})} >= 3 & ${LEN(${PEER})} <= 6]?intchk)
 ...
 same => n,Set(OUTFROM=${DB(SIP/gwin/${GW})})
 same => n,ExecIf($["${OUTFROM}" = ""]?Set(OUTFROM=${SRC}))
 same => n,Set(QUEUE_DST=${GW})
 same => n,Set(QUEUE_FROM=${OUTFROM})
 same => n,Set(QUEUE_BODY_B64=${BASE64_ENCODE(${MESSAGE(body)})})
 same => n,MessageSend(pjsip:${GW},sip:${OUTFROM}@sip.elfradio.net,sip:${GW}@sip.elfradio.net)
 same => n,Verbose(0, SMS out SRC=${SRC} GW=${GW} NUM=${SMS_NUMBER} from=${OUTFROM} status=${MESSAGE_SEND_STATUS})
 same => n,GotoIf($["${MESSAGE_SEND_STATUS}" = "SUCCESS"]?ok)
 same => n,AGI(sms-queue.agi,store)
 same => n,Verbose(0, SMS out queued SRC=${SRC} GW=${GW} qid=${SMS_QID})
 same => n(ok),Hangup()
 same => n(intchk),GotoIf($["${DB(SIP/is_gw/${PEER})}" = "1"]?invalid)
 same => n,GotoIf($["${PEER}" = "${SRC}"]?invalid)
 same => n,GotoIf($["${DB(SIP/sms/${PEER})}" != "1"]?invalid)
 same => n,Set(SG=${DB(SIP/group/${SRC})})
 same => n,Set(DG=${DB(SIP/group/${PEER})})
 same => n,Set(POL=${DB(SIP/gpol/${SG})})
 same => n,ExecIf($["${SG}" = ""]?Set(POL=self))
 same => n,GotoIf($["${POL}" = "all"]?intsend)
 same => n,GotoIf($["${SG}" = "${DG}"]?intsend)
 same => n,GotoIf($["${POL}" = "peers" & "${DB(SIP/gpeer/${SG}/${DG})}" = "1"]?intsend)
 same => n,Goto(invalid)
 same => n(intsend),Set(QUEUE_DST=${PEER})
 same => n,Set(QUEUE_FROM=${SRC})
 same => n,Set(QUEUE_BODY_B64=${BASE64_ENCODE(${MESSAGE(body)})})
 same => n,MessageSend(pjsip:${PEER},sip:${SRC}@sip.elfradio.net,sip:${PEER}@sip.elfradio.net)
 same => n,Verbose(0, SMS int SRC=${SRC} DST=${PEER} status=${MESSAGE_SEND_STATUS})
 same => n,GotoIf($["${MESSAGE_SEND_STATUS}" = "SUCCESS"]?ok)
 same => n,AGI(sms-queue.agi,store)
 same => n,Verbose(0, SMS int queued SRC=${SRC} DST=${PEER} qid=${SMS_QID})
 same => n,Goto(ok)
 same => n(invalid),Verbose(0, SMS out rejected SRC=${SRC} exten=${EXTEN})
 same => n,Hangup()
...
exten => _.,1,NoOp(Inbound SMS from ${MESSAGE(from)} to ${MESSAGE(to)} exten=${EXTEN})
 ...
 same => n,GotoIf($["${DB(SIP/sms/${DST})}" != "1"]?invalid)
 same => n,Set(GSM_SMS_SENDER=${FILTER(0-9+,${MESSAGE_DATA(X-GSM-CallerID)})})
 same => n,Verbose(0, SMS in callerhdr=${MESSAGE_DATA(X-GSM-CallerID)} filtered=${GSM_SMS_SENDER})
 same => n,GotoIf($[${LEN(${GSM_SMS_SENDER})} < 6]?nocaller)
 same => n,GotoIf($[${LEN(${GSM_SMS_SENDER})} > 16]?nocaller)
 ...
 same => n,Set(QUEUE_DST=${DST})
 same => n,Set(QUEUE_FROM=${SIP_SMS_SENDER})
 same => n,Set(QUEUE_BODY_B64=${BASE64_ENCODE(${MESSAGE(body)})})
 ...
 same => n,GotoIf($["${MESSAGE_SEND_STATUS}" = "SUCCESS"]?ok)
 same => n,AGI(sms-queue.agi,store)
 same => n,Verbose(0, SMS in queued GW=${GW} DST=${DST} qid=${SMS_QID})
 same => n(ok),Hangup()
 same => n(nocaller),Set(SIP_SMS_SENDER=anonymous)
 same => n,Verbose(0, SMS in missing callerid, still send GW=${GW} DST=${DST})
 same => n,Goto(send)
...
exten => s,1,Set(FROM=sip:300@sip.elfradio.net)
 ...
 same => n,Hangup()
exten => b,1,Set(FROM=${BASE64_DECODE(PHNpcDozMDBAc2lwLmVsZnJhZGlvLm5ldD4=)})
 same => n,Set(U=${CUT(FROM,@,1)})
 same => n,Set(U=${CUT(U,:,2)})
 same => n,Set(GW=${FILTER(0-9,${U})})
 same => n,Verbose(0, SMS parse bracket FROM=${FROM} U=${U} GW=${GW})
 same => n,Hangup()

[sms-flush]
exten => _X.,1,Answer()
 same => n,NoOp(SMS flush DST=${EXTEN} QID=${QUEUE_ID})
 same => n,Set(MESSAGE(body)=${BASE64_DECODE(${QUEUE_BODY_B64})})
 same => n,GotoIf($[${LEN(${MESSAGE(body)})} < 1]?bad)
 same => n,Set(QF=${FILTER(0-9A-Za-z+,${QUEUE_FROM})})
 same => n,GotoIf($["${QF}" = ""]?bad)
 same => n,MessageSend(pjsip:${EXTEN},sip:${QF}@sip.elfradio.net,sip:${EXTEN}@sip.elfradio.net)
 same => n,Verbose(0, SMS flush DST=${EXTEN} QID=${QUEUE_ID} status=${MESSAGE_SEND_STATUS})
 same => n,AGI(sms-queue.agi,ack)
 same => n,Hangup()
 same => n(bad),Verbose(0, SMS flush rejected DST=${EXTEN} QID=${QUEUE_ID})
 same => n,AGI(sms-queue.agi,ackfail)
 same => n,Hangup()
"""
