...
test('通信终端首次及刷新渲染均将连接入口置于标题右侧，设备名与历史记录保留右侧',()=>{
 ...
 const media={connectionControl(d){assert.equal(d,device);connects++;return '<button>连接</button>';},isActive:()=>false,preview:(d,h)=>h,controls:()=>'',feedback:()=>'<span class="media-feedback">正在连接…</span>',mount(){mounts++;}};
 ...
 ctx.gatewayDevice=d=>d?.product_id==='elfremote_gateway';
 ...
 assert.ok(box.innerHTML.indexOf('<button>连接</button>')<box.innerHTML.indexOf('正在连接…'));assert.ok(box.innerHTML.indexOf('正在连接…')<box.innerHTML.indexOf('remote-head-actions'));assert.equal((box.innerHTML.match(/media-feedback/g)||[]).length,1);
});
