import {faultRequire as need,faultTarget,faultCommand,faultReceipt,faultPending,faultTask,faultArchiveResult,faultArchiveQuery,faultNumber,FAULT_HEX} from './fault-contract.js';
...
function page(){
  ...
  if(p?.capacity){const x=p.capacity;html+=`<p class="muted">最近一次扫描 · ${date(x.capturedAtMs)}　活动 ${faultNumber(x.activeEvents)} / ${faultNumber(x.maxActiveEvents)}　已归档 ${faultNumber(x.archivedEvents)}　保留 ${faultNumber(x.retainedEvents)} / ${faultNumber(x.maxRetainedEvents)}　原件 ${faultNumber(x.retainedBytes)} / ${faultNumber(x.maxArchiveBytes)} 字节</p><p class="muted">索引错误 ${faultNumber(x.eventIndexErrors)}　限制：${e((x.admissionBlockedBy||[]).map(k=>phaseLabel[k]||k).join(' / ')||'未报告')}</p>`;}
  ...
  const id=c.selected,ev=s?.events?.[id];if(id){html+=`<div class="system-setting-section"><h4>所选事件 · ${e(id.slice(0,16))}</h4><div class="ops-actions">${action('query','核查详情',h)}${action('transfer','取回并校验原件',h||!ev?.query||ev.phase==='DONE')}${action('archive','确认归档',h||!ev?.proof||!['ARCHIVE','CONFIRM'].includes(ev.phase))}<span>${e(labels[ev?.phase]||'尚未取回')}</span></div>`;
    ...
    if(ev?.phase==='DONE')html+='<p>归档已独立确认；设备原件保留，仅释放活动名额，未删除原件。</p>';html+='</div>';
  }
  return html;
}
...
async function run(fn){const c=context();if(!c||c.busy)return;if(c.retryAt&&Date.now()<c.retryAt){c.message='请等待服务退避结束后继续';render();return;}c.busy=true;c.message='正在处理…';c.budget={requests:0,bytes:0,start:performance.now()};render();
  try{need(c.directory&&c.state,'请先选择本机保全目录');need(navigator.locks,'浏览器不支持本机工作流互斥');await navigator.locks.request('elfremote-fault-'+c.id,{ifAvailable:true},async lock=>{need(lock,'另一个页面正在处理此设备故障');const fresh=faultJson(await fileRead(c,'fault-web-state.json'));need(fresh.schemaVersion===1&&JSON.stringify(fresh.target)===JSON.stringify(c.state.target)&&fresh.tasks&&fresh.events,'本机工作流身份不符');c.state=fresh;await checkTarget(c);await fn(c);});}
  catch(error){c.message=error.message||'操作未完成，请继续原操作';}finally{c.busy=false;render();}
}
async function choose(){const c=context();if(!c||c.busy)return;try{
  need(window.showDirectoryPicker,'请使用支持本机目录访问的桌面浏览器');const target=faultTarget(window.currentDev()),directory=await showDirectoryPicker({mode:'readwrite',id:'elfremote-faults'});c.directory=directory;
  ...
  c.message='已恢复本机工作流';render();
  }catch(error){c.directory=null;c.state=null;c.message=error.name==='AbortError'?'已取消选择':error.message;render();}
}
...
async function transfer(c){const ev=selected(c);if(ev.receipt)faultReceipt(ev.receipt,ev.eventId);need(ev.query&&['COMPLETE','PARTIAL'].includes(ev.query.state?.phase||ev.query.phase),'事件尚未进入可导出终态，请先核查详情');
  ...
  if(ev.phase==='DOWNLOAD'){
    const taskId=ev.tasks.getFile.request.id,q=new URLSearchParams({device_id:c.id,task_id:taskId}),m=await request(c,'/api/elfremote/file-return?'+q),f=m.file,r=ev.receipt;
    ...
    ev.bundle||='bundle-'+ev.eventId+'-'+crypto.randomUUID()+'.zip';await save(c);
    let existing=false;try{const b=await fileRead(c,ev.bundle,8388608);existing=b.length===r.bytes&&await faultHash(b)===r.sha256;}catch(error){if(error.name!=='NotFoundError')throw error;}
    if(!existing){need(c.budget.requests<48,'本轮请求预算已到');c.budget.requests++;const res=await fetch('/api/elfremote/file-return?'+q+'&download=1',{credentials:'same-origin',redirect:'error',signal:AbortSignal.timeout(30000)});need(res.status===200,'故障包下载失败');const reader=res.body.getReader(),chunks=[];let length=0;
