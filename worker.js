...
function renderSipHtml() {
  return [
    ...
    '  if(SYNC && SYNC.error){ el.innerHTML="<span class=\\"bad\\">保存到交换机失败：<\\/span>"+SYNC.error; el.style.display="block"; return; }',
    '  if(SYNC && SYNC.pending && window._sipSaved){ el.innerHTML="<span class=\\"warn\\">正在把刚才的修改写到交换机…<\\/span>"; el.style.display="block"; return; }',
    '  if(SYNC && SYNC.pending){ el.innerHTML=""; el.style.display="none"; return; }',
