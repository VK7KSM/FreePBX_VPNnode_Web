...
const TOKEN_CACHE_MS = 60000;
let tokenCache = { at: 0, val: "" };

async function heartbeatToken(env) {
  if (tokenCache.val && (Date.now() - tokenCache.at) < TOKEN_CACHE_MS) return tokenCache.val;
  tokenCache.val = (await getStore(env, "sip_heartbeat_token")) || "";
...
const app = {
  ...
  async fetch(request, env, ctx) {
    ...
    if(pathname==='/api/devices/trajectory-media'&&method==='GET'){
      try{return json(await queryTrajectoryMedia(env.__storage,url));}catch(error){return json({ok:false,msg:error.message},400);}
    }
...
function publicDevice(d, modelName, model = {}) {
  ...
  return {
    ...
    status_only: d.status_only === true,
    managed_log_tasks: d.managed_log_tasks === true,
    managed_heal_tasks: d.managed_heal_tasks === true,
    ...
    managed_file_delete: d.managed_file_delete === true,
...
async function handleDeviceDelete(env, request) {
  try {
    ...
    if (removed.registration_version === 2) {
      ...
      removed.unpaired_at_ms = Date.now();
...
async function handleDeviceReport(env, request) {
  try {
    ...
    for (let i = 0; i < list.length; i++) {
      ...
      list[i].managed_file_delete = data.managed_file_delete === true;
    ...
    if ((!data.status_only || found.task?.managed_exec_v1 === true || found.task?.managed_log_v1 === true || found.task?.managed_heal_v1 === true || found.task?.managed_reboot_v1 === true || found.task?.managed_adbd_v1 === true || found.task?.managed_wifi_scan_v1 === true || found.task?.managed_wifi_config_v1 === true || found.task?.managed_alarm_v1 === true || found.task?.managed_locate_v1 === true || found.task?.managed_config_v1 === true || found.task?.managed_lost_v1 === true || found.task?.managed_pixel_companion_v1 === true) && found.task && repairExpired(found.task, now)
      ...
      found.task.detail = found.task.type===PIXEL_COMPANION_TASK_TYPE?'companion-task-expired':"expired";
      if(found.task.type===PIXEL_COMPANION_TASK_TYPE){found.task.params={};found.task.completed_at=new Date(now).toISOString();}
...
function verifyLocateTaskFromReport(device,record){
  const task=device?.task,location=record?.location;
  if(task?.type!=='locate_now'||task.state!=='success'||task.detail!=='location-sampled'
      ||task.result?.stage!=='location'||task.result?.action!=='sampled'||task.result.verified===true)return false;
  if(!location||!['gps','wifi','cell'].includes(location.source))return false;
  const created=Date.parse(task.created_at),sampled=Date.parse(record.sample_at),received=Date.parse(record.received_at);
  if(!Number.isFinite(created)||!Number.isFinite(sampled)||!Number.isFinite(received)
      ||sampled<created||sampled>received+5000)return false;
...
function addManagedTaskOffer(body, device, report, now) {
  ...
  if(device.enabled!==false&&isGateway(device)&&report.status_only===true&&report.managed_lost_message_v1===true
      &&device.task?.managed_lost_message_v1===true&&LOST_MESSAGE_TASK_TYPES.includes(device.task.type)&&shouldOfferRepair(device,now))
    body.managed_task={...repairOfferPayload(device.task),managed_lost_message_v1:true};
  if(device.enabled!==false&&isGateway(device)&&report.status_only===true&&report.managed_pixel_companion_v1===true
      &&device.task?.managed_pixel_companion_v1===true&&device.task.type===PIXEL_COMPANION_TASK_TYPE&&shouldOfferRepair(device,now))
    body.managed_task={...repairOfferPayload(device.task),managed_pixel_companion_v1:true};
...
async function handleElfEnqueueTask(env, request) {
  try {
    ...
    if(LOST_MESSAGE_TASK_TYPES.includes(data.type)&&(!isGateway(found)||found.managed_lost_message_v1!==true))
      return json({ok:false,msg:'当前 Pixel Gateway 尚未支持丢失信息控制'},409);
    if(data.type===PIXEL_COMPANION_TASK_TYPE&&(!isGateway(found)||found.managed_pixel_companion_v1!==true))
      return json({ok:false,msg:'当前 Pixel Gateway 尚未声明伴随组件暂存能力'},409);
    ...
    if(found.task && repairExpired(found.task,Date.now()) && ["pending","claimed","running"].includes(found.task.state)&&!holdNetworkTask(found.task)){
      ...
      if(found.task.type===PIXEL_COMPANION_TASK_TYPE){found.task.params={};found.task.detail='companion-task-expired';found.task.completed_at=new Date().toISOString();}
    }
...
function renderDevicesHtml() {
  return [
    ...
    '.btn-add{padding:.25rem .65rem;font-size:.8rem;height:28px}.devices-heading{display:flex;align-items:center;gap:6px;padding:.75rem .8rem 0}.devices-heading h3{margin:0 auto 0 0;font-size:.95rem}#refreshDevices{width:28px;height:28px;display:inline-flex;align-items:center;justify-content:center;padding:0}',
    '.btn-gray{padding:.4rem .8rem;background:#334155;color:#cbd5e1;border-radius:.5rem;cursor:pointer;border:none;font-size:.8rem}',
    '.btn-gray:disabled{opacity:.35;cursor:default}',
    ...
    '.dev-row{display:grid;grid-template-columns:12px minmax(0,1fr) auto;align-items:center;gap:.35rem;padding:.35rem .45rem;min-height:48px;box-sizing:border-box;border-radius:.5rem;cursor:pointer}.dev-identity{min-width:0;display:flex;flex-direction:column;align-items:flex-start;gap:2px}.dev-identity .dev-name{max-width:100%}.dev-identity .tag{white-space:nowrap;font-size:.6rem}.report-status{font-size:.65rem;color:#fbbf24;white-space:nowrap;cursor:pointer;line-height:1.4}.report-status:hover{text-decoration:underline}.report-status:focus-visible{outline:1px solid #fbbf24;outline-offset:3px}.report-status[aria-disabled=true]{color:#94a3b8;cursor:wait}',
    ...
    '.remote-title{display:flex;align-items:center;gap:8px;min-width:0;flex:1}.remote-head{height:32px}.remote-head-actions{flex-shrink:0;max-width:45%}.remote-title h3{flex-shrink:0}#serviceError{font-size:11px;font-weight:400;color:#f87171;line-height:1.4;min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}',
    ...
    '.ops-head-actions .device-action,.remote-title .device-action{display:inline-flex;align-items:center;justify-content:center;height:32px;box-sizing:border-box;padding:0 12px;border:0;border-radius:6px;color:#fff;font-size:12px;font-weight:500;line-height:16px;white-space:nowrap;cursor:pointer}.action-edit{background:#285d89}.action-disable{background:#77551f}.action-enable{background:#27634c}.action-pair{background:#167359}.action-unpair{background:#782f3a}.device-action:hover{filter:brightness(1.12)}.device-action:disabled{opacity:.45;cursor:default}',
    ...
    '.desktop-view{display:flex;flex-direction:column;height:340px;min-height:340px;max-height:340px;background:#000;border-top:1px solid #29364a}.desktop-stage{position:relative;flex:1;min-height:0}.desktop-screen{position:absolute;inset:0;display:flex;align-items:center;justify-content:center;outline:none;touch-action:none;user-select:none;cursor:crosshair}.desktop-canvas{max-width:100%;max-height:100%;width:auto;height:auto;display:block}.desktop-tools{position:absolute;left:0;top:0;bottom:0;width:48px;display:flex;flex-direction:column;justify-content:center;gap:4px;padding:6px 6px;box-sizing:border-box;background:rgba(15,23,42,.85);border-right:1px solid #29364a;overflow:hidden}.desktop-tools button{width:36px;height:36px;min-height:36px;padding:0;display:inline-flex;align-items:center;justify-content:center;background:#1e293b;color:#e2e8f0;border:0;border-radius:8px;cursor:pointer;flex-shrink:0}.desktop-tools button svg{width:20px;height:20px}.desktop-tools button:hover,.desktop-tools button:focus-visible{background:#3b82f6;color:#fff}.desktop-unfold{position:absolute;left:0;top:50%;transform:translateY(-50%);width:26px;height:64px;padding:0;border:0;border-radius:0 8px 8px 0;background:rgba(51,65,85,.9);color:#e2e8f0;cursor:pointer;display:flex;align-items:center;justify-content:center}.desktop-unfold svg{width:18px;height:18px}.desktop-unfold[hidden]{display:none}.desktop-info{position:absolute;top:8px;right:8px;display:flex;flex-direction:column;gap:2px;padding:6px 10px;border-radius:6px;background:rgba(15,23,42,.85);color:#cbd5e1;font-size:12px;line-height:1.5;max-width:60%;pointer-events:none}.desktop-info[hidden]{display:none}.desktop-info span{white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.desktop-foot{display:none}',
    '.share-mode .btn-add,.share-mode .ops-head-actions .action-disable,.share-mode .ops-head-actions .action-enable,.share-mode .ops-head-actions .action-unpair,.share-mode .ops-head-actions .action-pair,.share-mode .fn-btn[data-fn=model],.share-mode .admin-only{display:none!important}.share-mode #shareDeviceName{font-weight:700;font-size:1.05rem;white-space:nowrap}.share-mode .share-brand-sub{color:#93c5fd;font-size:.85rem;font-weight:600;white-space:nowrap;margin-left:.4rem}.file-send-wrap .file-send-dialog.share-dialog{width:900px;max-width:calc(100vw - 32px);max-height:none;overflow:visible;font-size:14px;color:#e2e8f0}.share-dialog h3{font-size:16px}.share-dialog .btn-close.share-close{background:#7f1d1d;border:1px solid #b91c1c;color:#fecaca;width:28px;height:28px;border-radius:6px;font-size:18px;line-height:1}.share-form-row{display:flex;align-items:center;gap:12px;flex-wrap:nowrap}.share-form-row label{display:flex;align-items:center;gap:10px;font-size:14px;color:#e2e8f0;white-space:nowrap}.share-form-row label.share-ttl{margin-left:20px}.share-form-row input.inp,.share-form-row select.inp{box-sizing:border-box;width:200px;height:36px;font-size:14px;line-height:34px;padding:0 10px;margin:0}.share-form-actions{margin-left:auto;display:flex;gap:8px}.share-form-row button{height:36px;font-size:14px;padding:0 18px;white-space:nowrap}.share-edit{border:1px solid #29364a;border-radius:8px;padding:12px 14px;margin-top:14px;min-width:0}.share-edit legend{font-size:13px;color:#94a3b8;padding:0 6px}.share-list-title{margin:18px 0 8px;font-size:13px;color:#94a3b8}.share-list{display:flex;flex-direction:column;gap:8px;max-height:44vh;overflow:auto}.share-item{display:flex;align-items:center;gap:14px;padding:10px 12px;border:1px solid #29364a;border-radius:8px;background:#0f172a}.share-item.on{border-color:#3b82f6;background:#1e293b}.share-item-main{flex:1;min-width:0;display:flex;flex-direction:column;gap:4px}.share-item-link{display:flex;align-items:center;gap:8px;min-width:0}.share-link{font-family:ui-monospace,Consolas,monospace;font-size:14px;color:#93c5fd;cursor:pointer;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.share-link:hover{text-decoration:underline}.share-item-meta{font-size:13px;color:#cbd5e1;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.share-delete{flex-shrink:0;padding:6px 16px;min-height:32px;font-size:13px}.share-qr-pop{position:fixed;z-index:3000;padding:8px;background:#fff;border-radius:8px;box-shadow:0 8px 24px rgba(0,0,0,.5);pointer-events:none}.share-qr-pop svg{width:180px;height:180px;display:block}.share-qr-pop[hidden]{display:none}.share-empty{color:#94a3b8;padding:10px 4px}.share-error{margin:8px 0 0;color:#fbbf24}.share-lock-tag{margin-left:10px;padding:2px 8px;border-radius:5px;background:#3b2f0b;color:#fbbf24;font-size:12px;white-space:nowrap}.action-share{background:#3b6fa8}',
    ...
    '.account-tabs{display:flex;gap:8px;margin-bottom:16px}.account-tabs button{font-size:12px;font-weight:400;padding:5px 14px;border-radius:6px}.account-tabs button.active{background:#253e60;color:#b9d8ff}.account-fields{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:12px 18px;max-width:680px}.account-fields>label{display:grid;gap:5px;color:#aebcce}.account-fields .inp{width:100%;font-size:12px;height:34px}.account-fields .ops-actions{grid-column:1/-1;flex-wrap:wrap}.account-fields [role=status]{color:#aebcce}@media(max-width:600px){.account-fields{grid-template-columns:1fr}}',
    ".trajectory-panel{font-size:12px;color:#d4deec;line-height:1.7;min-width:0}.trajectory-toolbar,.trajectory-presets,.trajectory-dates,.trajectory-playbar{display:flex;align-items:center;gap:8px;flex-wrap:wrap}.trajectory-toolbar{justify-content:space-between;margin-bottom:18px}.trajectory-panel button{font-size:12px;font-weight:400;cursor:pointer;border:1px solid #354359;background:#243246;color:#c6d2e3;border-radius:6px;padding:5px 11px;min-height:30px}.trajectory-panel button:hover{background:#304158}.trajectory-panel button.active{color:#d7e8ff;background:#304d70;border-color:#456789}.trajectory-panel button:disabled{opacity:.45;cursor:default}.trajectory-dates input{width:134px;height:30px;color:#d4deec;background:#172335;border:1px solid #354359;border-radius:6px;padding:4px 8px;font:inherit;color-scheme:dark}.trajectory-playbar{margin:0 0 16px;gap:6px}.trajectory-playbar #trajectoryTime{margin-left:8px;color:#cbd5e1;font-variant-numeric:tabular-nums}.trajectory-count{margin-left:auto;color:#8799b0}.trajectory-axis{position:relative;margin:0 12px;height:58px;border-bottom:1px solid #40516a;touch-action:pan-y}.trajectory-tick{position:absolute;top:0;height:100%;border-left:1px solid #2a384c;pointer-events:none}.trajectory-tick time{position:absolute;top:0;left:0;transform:translateX(-50%);color:#8295af;font-size:11px;white-space:nowrap}.trajectory-tick:first-child time{transform:none}.trajectory-tick:nth-child(5) time{transform:translateX(-100%)}.trajectory-panel .trajectory-point{position:absolute;top:43px;transform:translateX(-50%);border:1px solid #829fbf;border-radius:50%;background:#668bb7;min-height:0;padding:0;width:7px;height:7px;line-height:17px;font-size:13px;z-index:2}.trajectory-panel .trajectory-point.selected{outline:2px solid #f5c451;outline-offset:3px;z-index:5}#trajectoryCursor{position:absolute;top:21px;bottom:0;border-left:1px dashed #f5c451;pointer-events:none;opacity:.5}.trajectory-slider{display:block;width:100%;height:14px;accent-color:#93b4d8;cursor:ew-resize;margin:8px 0 0}.trajectory-detail-list{margin-top:18px;border-top:1px solid #29374a;padding-top:7px}.trajectory-detail-row{display:flex;align-items:center;gap:14px;min-height:34px;font-size:12px}.trajectory-field{width:42px;flex-shrink:0;color:#8295ad;font-size:11px}.trajectory-detail-row .traffic-link{margin-left:8px;min-height:0}.trajectory-note{color:#91a2b8;font-size:11px;margin:0 0 8px}.trajectory-empty{height:180px;display:flex;align-items:center;justify-content:center;color:#8fa0b7}.trajectory-error{color:#f3a6a6}.trajectory-preview-caption{display:flex;gap:8px;align-items:center;justify-content:space-between;font-size:11px;color:#9db0c8;margin:-2px 0 7px}.trajectory-player video{width:100%;height:100%;object-fit:contain}.trajectory-player audio{position:absolute;bottom:34px;width:92%;height:35px;z-index:2}.trajectory-player canvas{width:94%;height:55%;position:absolute;top:7%}.trajectory-player .photo-time{bottom:9px}.trajectory-play-error{position:absolute;top:40%;color:#f3a6a6;font-size:12px;z-index:3;background:#172335;padding:6px 10px;border-radius:5px}.trajectory-tooltip{background:#18283b!important;border:1px solid #526079!important;color:#e1eaf6!important;box-shadow:none!important;font-size:11px!important}.trajectory-photo>span{font-size:12px}.trajectory-photo .photo-nav{font-size:26px}.trajectory-panel .traffic-link{border:none;background:transparent;color:#a4c3e6;padding:3px 5px}.trajectory-panel .traffic-link:hover{background:transparent;color:#d9e9ff}@media(max-width:700px){.trajectory-dates{width:100%}.trajectory-playbar #trajectoryTime{order:2;width:100%;margin:3px 0}.trajectory-count{font-size:11px;order:2;margin-left:0;flex-basis:40%}.trajectory-playbar>.traffic-link{order:2;margin-left:auto}.trajectory-playbar #trajectoryTime{order:3}.trajectory-playbar{gap:5px}}" +
    ".file-manager-page{font-size:12px;line-height:1.8;min-width:0}.file-manager-page button{font-size:12px;font-weight:400;min-height:30px;padding:5px 10px;border-radius:6px;white-space:nowrap}.file-manager-toolbar,.file-manager-path,.file-manager-pagination{display:flex;align-items:center;gap:8px;flex-wrap:wrap}.file-manager-count{color:#94a3b8;margin-left:auto;white-space:nowrap}.file-toolbar-divider{width:1px;height:18px;background:#334155;margin:0 3px}.file-tool,.file-delete{background:#233044;color:#d4deec;border:1px solid #344258;cursor:pointer}.file-delete{color:#eab4b4}.file-tool:hover:not(:disabled){background:#304158}.file-delete:hover:not(:disabled){background:#632f37;color:white}.file-manager-page button:disabled{opacity:.45;cursor:default}.file-manager-path{margin:14px 0 8px}.file-manager-page input.inp{font-size:12px;height:32px;min-width:0;padding:5px 10px}.file-manager-path input{flex:1}.file-manager-table{width:100%;table-layout:fixed;border-collapse:collapse}.file-manager-table th,.file-manager-table td{font-size:12px;font-weight:400;text-align:left;border-bottom:1px solid #29364a;padding:10px 8px}.file-manager-table th{color:#94a3b8}.file-manager-table .file-select{width:28px;padding-left:4px}.file-manager-table input[type=checkbox]{accent-color:#60a5fa;width:13px;height:13px;vertical-align:middle}.file-manager-table .file-size{text-align:right;width:84px;white-space:nowrap}.file-manager-table .file-type{width:96px;color:#aebcce}.file-manager-table .file-modified{width:140px;white-space:nowrap;color:#aebcce}.file-manager-table .file-permissions{width:143px;white-space:nowrap;font-size:11px;color:#aebcce;font-family:ui-monospace,Consolas,monospace}.file-name a{display:flex;align-items:center;gap:9px;color:#d4deec;text-decoration:none;min-width:0}.file-name a>span:last-child{overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.file-entry-icon{display:inline-block;position:relative;flex-shrink:0}.file-entry-icon.folder{width:18px;height:12px;border-radius:2px;background:#b7a273;margin-top:3px}.file-entry-icon.folder:before{content:\"\";position:absolute;left:0;top:-3px;width:8px;height:4px;background:#cdb985;border-radius:2px 2px 0 0}.file-entry-icon.document{width:12px;height:15px;border:1px solid #93a6be;border-radius:2px;margin:0 3px}.file-entry-icon.document:after{content:\"\";position:absolute;left:2px;right:2px;top:6px;height:1px;background:#7389a4;box-shadow:0 3px #7389a4}.file-manager-table tr:hover td{background:rgba(148,163,184,.04)}.file-manager-table .selected{background:rgba(96,165,250,.12)}.file-manager-table .file-empty{text-align:center;padding:35px 10px;color:#94a3b8}.file-manager-footer{display:flex;align-items:center;flex-wrap:wrap;justify-content:space-between;gap:14px;min-height:32px;margin-top:14px}.file-operation-status{display:flex;align-items:center;gap:10px;min-width:0;color:#aebcce}.file-operation-status.error{color:#f1a5a5}.file-operation-status.success{color:#73c8a5}.file-operation-status progress{width:95px;height:4px;accent-color:#60a5fa}.file-manager-pagination{margin-left:auto;color:#94a3b8}.file-manager-pagination>span{margin-right:7px;white-space:nowrap}.file-directory-path{display:flex;align-items:center;gap:10px;overflow-wrap:anywhere}.file-directory-list{max-height:280px;overflow:auto;min-height:100px}.file-directory-item{display:block;width:100%;background:transparent;border:0;border-bottom:1px solid #29364a;color:#d4deec;text-align:left;padding:8px;cursor:pointer}.file-directory-item:hover{background:#233044}.file-conflict-actions{display:flex;gap:10px}.file-operation-dialog input.inp{box-sizing:border-box;width:100%}@media(max-width:900px){.file-manager-table .file-permissions{width:115px;font-size:10px}.file-manager-table .file-type{width:75px}.file-manager-table .file-modified{width:125px;font-size:11px}}@media(max-width:650px){.file-manager-page button{padding:5px 7px}.file-manager-table th,.file-manager-table td{padding:8px 4px}.file-manager-table .file-type,.file-manager-table .file-permissions{display:none}.file-manager-table .file-size{width:65px}.file-manager-table .file-modified{width:110px;font-size:10px}.file-manager-count{width:100%;margin-left:0}.file-manager-pagination{gap:5px}}",
    ...
    '.adb-box{border:1px solid #1e293b;border-radius:.55rem;overflow:hidden;background:#020617}',
    '.adb-term{height:340px;overflow:auto;background:#020617;color:#d1fae5;font:13px/1.45 ui-monospace,SFMono-Regular,Consolas,monospace;padding:.75rem .9rem;border:0;border-radius:0;white-space:pre-wrap;word-break:break-all;margin:0}',
    '.adb-in{color:#93c5fd}.adb-out{color:#bbf7d0}.adb-sys{color:#94a3b8}',
    '.adb-row{display:flex;align-items:center;gap:.5rem;padding:.4rem .65rem;border-top:1px solid #1e293b;background:#01040d}',
    '.adb-prompt{color:#4ade80;font:13px ui-monospace,SFMono-Regular,Consolas,monospace;flex-shrink:0}',
    ...
    '.dpin-wrap{background:transparent;border:0;pointer-events:none!important}.dpin-card,.dpin-dot{pointer-events:auto!important;cursor:pointer}',
    '.dpin{display:flex;align-items:center;gap:5px;pointer-events:none}',
    '.dpin-dot{width:6px;height:6px;border-radius:50%;flex-shrink:0}',
    ...
    '<div class="remote-stage"><section id="remoteConsole" class="card remote-console" aria-label="通信终端"></section><div class="card remote-map">',
    ...
    '<\/div>',
    '<\/div><\/main>',
    '<div id="trafficHistoryWrap" class="modal-bg" style="display:none" onclick="if(event.target===this)closeTrafficHistory()"><div class="card modal-card"><div id="trafficHistoryBody"></div></div></div>',
