...
function fixture(){
 ...
 class Peer{constructor(){peers.push(this);this.connectionState='new';this.transceivers=[];}addTransceiver(t){const row={mid:String(this.transceivers.length),sender:{track:typeof t==='string'?null:t,async replaceTrack(track){this.track=track;}}};this.transceivers.push(row);return row;}getTransceivers(){return this.transceivers;}async createOffer(){return {type:'offer',sdp:'local'};}async createAnswer(){return {type:'answer',sdp:'answer'};}async setLocalDescription(d){this.localDescription={...d,toJSON:()=>({...d})};}async setRemoteDescription(d){this.remoteDescription=d;}close(){this.connectionState='closed';}connect(){this.connectionState='connected';this.onconnectionstatechange();}remote(kind){this.ontrack({track:track(kind)});}}
...
test('未点击连接不能启动新协议功能，建链仅生成软件静音，不申请真实麦克风',async()=>{
 const f=fixture();try{
  ...
  await f.connect();assert.equal(f.captureCount(),0);assert.equal(f.contexts.length,1);assert.equal(f.requests.length,1);assert.equal(f.requests[0].init.body.includes('prepare'),true);
...
test('连续两次PTT只建一次会话，停止恢复待命不关闭PC或WS',async()=>{
 const f=fixture();try{
  ...
  ws.emit({type:'ready',operation:1});await flush();await f.media.stop();assert.equal(f.peers[0].connectionState,'connected');assert.equal(ws.readyState,1);assert.equal(f.locals[0].getTracks()[0].readyState,'ended');
  ...
  await f.media.start('ptt');assert.equal(ws.sent.filter(p=>p.type==='activate').at(-1).operation,2);assert.equal(f.requests.length,1);assert.equal(f.peers.length,1);
...
test('麦克风授权挂起时取消，不下发停止空操作，迟到采音不能复活或消耗编号',async()=>{
 const f=fixture();try{
  const ws=await f.connect(),release=f.deferCapture(),pending=f.media.start('ptt');await flush();await f.media.stop();release();await pending;
  assert.equal(ws.sent.some(p=>p.type==='activate'||p.type==='deactivate'),false);assert.equal(f.locals[0].getTracks()[0].readyState,'ended');assert.equal(f.media.isActive(),false);
  await f.media.start('ptt');assert.equal(ws.sent.find(p=>p.type==='activate').operation,1);
 }finally{await f.dispose();}
});
