...
test('未点击连接不能启动新协议功能，建链不申请麦克风也不创建音频上下文',async()=>{
 const f=fixture();try{
  ...
  await f.connect();assert.equal(f.captureCount(),0);assert.equal(f.contexts.length,0);assert.equal(f.requests.length,1);assert.equal(f.requests[0].init.body.includes('prepare'),true);
