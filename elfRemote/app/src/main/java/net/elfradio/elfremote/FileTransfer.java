    ...
    synchronized void receive(JSONObject offer,String device,String token)throws Exception {
        ...
        new Thread(()->{
            ...
            try {
                ...
                progress(id,"claimed","设备已接收文件任务",null);
                ...
                if(core==null){
                    ...
                    progress(id,"running","正在接收文件",null);
                    long lastProgress=0;
                    ...
                    check(offer);
                    progress(id,"running","接收完成，正在校验并保存文件",null);
                ...
                try{
                    ...
                    if(cancelled||error instanceof Permanent||count>=3||offer.optLong("expires_at")<=System.currentTimeMillis()){
                        progress(id,"failed",cancelled?"文件接收已停止":error.getMessage(),null);active.delete();new File(dir,"payload.part").delete();
                    }else{progress(id,"running","接收中断，稍后从断点继续",null);new WakeScheduler(context).schedule("file-transfer",60000L*count);}
