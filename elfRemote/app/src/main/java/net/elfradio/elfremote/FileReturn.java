...
final class FileReturn {
    ...
    private long localProgressAt,localDone,localSize;
    ...
    synchronized void receive(JSONObject offer,String device,String token)throws Exception{
        ...
        currentOffer=offer;localProgressAt=0;localDone=0;localSize=0;
        ...
        new Thread(()->{
            ...
            try{
                ...
                progress(id,"claimed","设备已接收取回文件任务",null);progress(id,"running","正在准备文件快照",null);
                ...
                long size=core.getLong("bytes");localSize=size;String hash=core.getString("sha256");if(!snapshot.isFile()||snapshot.length()!=size)throw new Permanent("文件快照不可用");
                ...
                try(InputStream in=new FileInputStream(snapshot)){
                    for(int index=0;sent<size;index++){
                        ...
                        if(old==null)upload(url+"&part="+index+"&sha256="+partHash,token,p,bytes,offer,sent);
                        else RuntimeLog.event("file_return_reuse task="+id+" part="+index+" bytes="+count);
                        sent+=count;localDone=sent;if(System.currentTimeMillis()-last>30000){progress(id,"running","上传到服务器 "+sent*100/Math.max(1,size)+"%",null);last=System.currentTimeMillis();}
                    }
                }
                ...
                progress(id,"success","发送完成",new JSONObject().put("action","uploaded").put("bytes",size).put("sha256",hash));cleanup(active,dir);
            }catch(Paused e){try{progress(id,"running","已暂停，等待允许的网络后继续发送",null);}catch(Exception ignored){}new WakeScheduler(context).schedule("file-return",900000L);
                ...
                try{
                    ...
                    if(cancelled||error instanceof Permanent||count>=3||offer.optLong("expires_at")<=System.currentTimeMillis()){
                        stopSnapshot(id);
                        progress(id,"failed",cancelled?"文件取回已停止":error.getMessage(),null);cleanup(active,dir);
                    }else{progress(id,"running","上传中断，稍后继续取回",null);new WakeScheduler(context).schedule("file-return",count*60000L);}
    ...
    private void progress(String id,String state,String detail,JSONObject result)throws Exception{
        FileInbox.progress(context,id,currentOffer.optJSONObject("params").optString("path"),"send",state,detail,localSize,
                FileTransferStatus.percent(localDone,localSize),currentOffer.optLong("expires_at"));
    ...
    private void localProgress(long done){
        ...
        FileInbox.progress(context,current,currentOffer.optJSONObject("params").optString("path"),"send","running","正在发送文件",localSize,
                FileTransferStatus.percent(done,localSize),currentOffer.optLong("expires_at"));
    }
    ...
    private void upload(String url,String token,JSONObject p,byte[] bytes,JSONObject offer,long begin)throws Exception{
        Network n=network(p);HttpURLConnection c=open(url,token,p,"PUT",bytes.length);try{c.setRequestProperty("Content-Type","application/octet-stream");try(OutputStream out=c.getOutputStream()){for(int offset=0;offset<bytes.length;offset+=8192){check(offer);if(n==null||!n.equals(network(p)))throw new Paused();out.write(bytes,offset,Math.min(8192,bytes.length-offset));localProgress(begin+Math.min(offset+8192,bytes.length));}}response(c);}finally{c.disconnect();connection=null;}
    }
