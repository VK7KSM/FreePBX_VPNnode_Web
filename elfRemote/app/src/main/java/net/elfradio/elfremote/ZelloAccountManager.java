...
final class ZelloAccountManager {
    ...
    static JSONObject apply(File folder,JSONObject params)throws Exception {
        ...
        try {
            ...
            RescueFiles.write(marker,new JSONObject().put("asleep",asleep).put("granted_microphone",mic).put("previous_username",params.optString("previous_username","")).toString());
            ...
            if(marker.isFile())try{if(restoreUi(folder,ok)&&!ok)output+="；已恢复原Zello账号";}catch(Exception error){ok=false;output+="；原账号或界面恢复未完成";}
        }
    ...
    private static boolean restoreUi(File folder,boolean success)throws Exception {
        ...
        try {
            if(!success){
                run(folder,tag+"-wake","input keyevent 224");
                ...
                String previous=state.optString("previous_username","");
                if(!previous.isEmpty())try(AccountUi ui=new AccountUi()){
                    boolean clicked=false;
                    for(int i=0;i<25;i++){
                        try{if(ui.accountVisible(previous)){resumed=true;break;}if(!clicked)clicked=ui.clickText(previous);}catch(IOException pending){}
                        ui.advanceOnboarding();Thread.sleep(800);
                    }
                    if(!resumed)throw new IOException("尚未恢复原Zello账号主页面");
                }
            }
        }finally{if(state.optBoolean("asleep"))run(folder,tag+"-sleep","input keyevent 223");}
