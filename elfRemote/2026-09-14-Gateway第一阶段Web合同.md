...
## 2026-09-14 通信终端布局更正（D22_Web_Dev）

按用户最新要求，更正前文隐藏网关媒体区域的设计：Pixel 保留统一的通信终端标题、连接入口位置、设备名称、历史记录、预览窗口、六个功能按钮和当日流量；移除网关运行、SIP 注册和通话忙闲的额外状态行。设备产品差异不能改变这个区域的布局。

PTT、电话、麦克风、拍照和录像始终禁用，不能因连接状态或误报媒体能力而开启。警报只按真实的 `managed_alarm_tasks` 能力开放，使用 `play_alarm` 普通任务；设备报告播放中时同一按钮切换为停止警报并发送 `stop_alarm`，待处理时避免重复提交。当前网关不支持媒体会话，因此连接入口保留但禁用，不虚构已连接。无照片时显示暂无上报照片，不能停留在读取中。切换到网关时沿用媒体模块的旧设备会话清理。

后台产品身份、更新通道、媒体拒绝和其他管理能力门均保持原合同。仅修改 Web，不发布、安装或分配 Pixel APK。验证包含完整布局、五项禁用、警报能力及任务路由、停止和待处理状态，浏览器只用合成数据拦截按钮，不触发真实警报。


## 2026-09-14 功能设置入口更正（D22_Web_Dev）

按用户最新要求，Pixel 和其他型号均显示完整八个功能入口：远程终端、更新客户端、系统配置、通信录、设备轨迹、文件管理、丢失模式、添加型号。此前按能力隐藏入口及切换型号后自动改选设备轨迹的设计撤销。功能页可以打开，执行按钮仍由真实能力控制，尤其不能开放未实现的更新、命令、联系人写入及丢失擦除。通信终端的五项媒体禁用与普通警报任务合同保持不变。本次仅修复公共页面，不执行设备任务。
