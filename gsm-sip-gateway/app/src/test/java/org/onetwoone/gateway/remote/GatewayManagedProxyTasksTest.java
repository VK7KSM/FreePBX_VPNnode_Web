...
public class GatewayManagedProxyTasksTest {
    ...
    @Test public void acceptsStrictConfigureContract()throws Exception {JSONObject task=base("configure_proxy").put("params",new JSONObject()
            .put("url","https://v.elfradio.net/api/elfremote/proxy-config/task-1?token=opaque").put("size",123).put("sha256",HASH));
    ...
    @Test public void rejectsUnknownAndOverBroadContracts()throws Exception {JSONObject[] invalid=new JSONObject[]{base("unknown"),base("start_proxy").put("params",new JSONObject().put("extra",true)),
            base("configure_proxy").put("params",new JSONObject().put("url","https://evil.example/api/elfremote/proxy-config/task-1").put("size",123).put("sha256",HASH)),
            base("configure_proxy").put("params",new JSONObject().put("url","https://v.elfradio.net/api/elfremote/proxy-config/task-1").put("size",GatewayProxyPolicy.MAX_CONFIG_BYTES+1).put("sha256",HASH))};
