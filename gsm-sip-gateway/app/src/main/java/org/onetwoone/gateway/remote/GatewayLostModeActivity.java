...
public final class GatewayLostModeActivity extends Activity {
    ...
    @Override protected void onResume(){super.onResume();if(!active()){finishAndRemoveTask();return;}resumed=true;showSystemBars();}
    ...
    private void render(){if(!active()){finish();return;}String message=getSharedPreferences("gateway-lost-display",MODE_PRIVATE).getString("message","");
        ...
        root.addView(title,new LinearLayout.LayoutParams(-1,-2));root.addView(body,new LinearLayout.LayoutParams(-1,-2));setContentView(root);showSystemBars();}
    ...
    private void showSystemBars(){getWindow().getDecorView().setSystemUiVisibility(View.SYSTEM_UI_FLAG_VISIBLE);}
}
