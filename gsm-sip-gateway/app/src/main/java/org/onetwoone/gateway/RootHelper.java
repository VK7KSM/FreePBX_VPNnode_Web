...
public class RootHelper {
    ...
    /**
     * Verify that a pre-installed, least-privilege policy grants this app access
     * to only the selected ALSA nodes. This method intentionally never changes
     * SELinux mode or filesystem permissions.
     */
    public static boolean verifyAlsaAccess(int card, int captureDevice, int playbackDevice) {
        String selinuxMode = execRoot("getenforce");
        if (selinuxMode == null || !"Enforcing".equalsIgnoreCase(selinuxMode.trim())) {
            Log.e(TAG, "Refusing direct ALSA access: SELinux must remain Enforcing");
            return false;
        }

        File captureNode = new File(getAlsaPcmNodePath(card, captureDevice, true));
        File playbackNode = new File(getAlsaPcmNodePath(card, playbackDevice, false));
        if (!hasDirectReadWriteAccess(captureNode) || !hasDirectReadWriteAccess(playbackNode)) {
            Log.e(TAG, "Direct ALSA access is not authorized for the selected PCM nodes. " +
                "Install a Pixel-specific SELinux policy or audio broker; global chmod is forbidden.");
            Log.e(TAG, "capture=" + describeAccess(captureNode) +
                ", playback=" + describeAccess(playbackNode));
            return false;
        }

        Log.i(TAG, "ALSA access verified with SELinux Enforcing");
        return true;
    }

    static String getAlsaPcmNodePath(int card, int device, boolean capture) {
        return "/dev/snd/pcmC" + card + "D" + device + (capture ? "c" : "p");
    }

    private static boolean hasDirectReadWriteAccess(File node) {
        return node.exists() && node.canRead() && node.canWrite();
    }

    private static String describeAccess(File node) {
        return node.getAbsolutePath() + " [exists=" + node.exists() +
            ", read=" + node.canRead() + ", write=" + node.canWrite() + "]";
    }
