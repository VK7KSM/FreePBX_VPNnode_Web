...
public class GsmAudioNative {
     ...
     */
    public static boolean setupQualcommCapture(int card, String captureRoute) {
        // Enable VOC_REC downlink only (UL would capture Incall_Music causing echo)
        return setMixerControl(card, captureRoute + " Mixer VOC_REC_DL", 1);
    }
     ...
     */
    public static boolean setupQualcommPlayback(int card, String injectionRoute) {
        // Enable Incall_Music injection for both SIM slots
        boolean ok = setMixerControl(card, "Incall_Music Audio Mixer " + injectionRoute, 1);
        setMixerControl(card, "Incall_Music_2 Audio Mixer " + injectionRoute, 1);
        return ok;
    }
     ...
     */
    public static boolean teardownQualcommMixer(int card, String captureRoute,
                                                 String injectionRoute) {
        boolean ok = true;
        ok &= setMixerControl(card, captureRoute + " Mixer VOC_REC_DL", 0);
        // VOC_REC_UL not used (causes echo)
        ok &= setMixerControl(card, "Incall_Music Audio Mixer " + injectionRoute, 0);
        setMixerControl(card, "Incall_Music_2 Audio Mixer " + injectionRoute, 0);
        return ok;
    }

    @Deprecated
    public static boolean teardownQualcommMixer(int card, String multimediaRoute) {
        return teardownQualcommMixer(card, multimediaRoute, multimediaRoute);
    }
