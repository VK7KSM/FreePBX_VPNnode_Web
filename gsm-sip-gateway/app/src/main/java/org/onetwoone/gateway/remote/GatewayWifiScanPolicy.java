...
final class GatewayWifiScanPolicy {
    ...
    static JSONArray networks(JSONArray input) throws Exception {
        ...
        for(int i=0;i<input.length();i++){
            JSONObject entry=input.getJSONObject(i);String ssid=entry.getString("ssid"),sec=entry.getString("sec");int rssi=entry.getInt("rssi");
            if(ssid.isEmpty()||ssid.getBytes(StandardCharsets.UTF_8).length>32||rssi < -127||rssi>0)continue;
            String key=ssid+'\0'+sec;JSONObject old=strongest.get(key);
            if(old==null||rssi>old.getInt("rssi"))strongest.put(key,entry);
        }
        ArrayList<JSONObject> sorted=new ArrayList<>(strongest.values());
        sorted.sort((a,b)->Integer.compare(b.optInt("rssi"),a.optInt("rssi")));
        JSONArray result=new JSONArray();for(int i=0;i<Math.min(30,sorted.size());i++)result.put(sorted.get(i));return result;
    }
    private GatewayWifiScanPolicy() {}
}
