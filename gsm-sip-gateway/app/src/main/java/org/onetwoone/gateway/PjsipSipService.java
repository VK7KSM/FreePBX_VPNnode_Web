...
public class PjsipSipService extends Service implements SipCallService {
    ...
    private static final long SIP_MESSAGE_TIMEOUT_MS = 30_000L;
    private final List<PendingSipMessage> pendingSipMessages = new ArrayList<>();

    private static class PendingSipMessage {
        final long smsId;
        final String toUri;
        final String body;
        final Buddy buddy;

        PendingSipMessage(long smsId, String toUri, String body, Buddy buddy) {
            this.smsId = smsId;
            this.toUri = toUri;
            this.body = body;
            this.buddy = buddy;
    ...
    public void onCreate() {
        ...
        // This handset is a headless gateway, not a local telephone.
        GatewayInCallService.configureSilentGatewayAudio(this);
    ...
    private void initializeManagers() {
        ...
        powerController.acquireWifiHighPerformanceLock();
        powerController.disableBatteryOptimizationsAsync();

        // SIP components
        endpointManager = new SipEndpointManager(this, config);
    ...
    private final SipAccountManager.AccountListener accountListener = new SipAccountManager.AccountListener() {
        ...
        @Override
        public void onInstantMessageStatus(String to, String body, int code, String reason) {
            mainHandler.post(() -> handleSipMessageStatus(to, body, code, reason));
        }
    };
    ...
    public void onDtmfDigit(GatewayCall call, String digit) {
        mainHandler.post(() -> callManager.onDtmfDigit(call, digit));
    }
    ...
    public void onGsmCallStateChanged(android.telecom.Call call, int state) {
        if (state == android.telecom.Call.STATE_DIALING ||
            ...
            audioBridge.startRingback();
        } else if (state == android.telecom.Call.STATE_ACTIVE) {
            audioBridge.stopRingback();
    ...
    private void sendSipMessage(String toExt, String gsmSender, String body, long smsId, int simSlot) {
        ...
        Buddy buddy = null;
        PendingSipMessage pending = null;
        try {
            ...
            pending = new PendingSipMessage(smsId, toUri, body, buddy);
            synchronized (pendingSipMessages) {
                pendingSipMessages.add(pending);
            }

            Log.d(TAG, "sendSipMessage: Calling buddy.sendInstantMessage for smsId=" + smsId);
            buddy.sendInstantMessage(prm);

            PendingSipMessage timeoutTarget = pending;
            mainHandler.postDelayed(() -> handleSipMessageTimeout(timeoutTarget), SIP_MESSAGE_TIMEOUT_MS);
            Log.i(TAG, "SIP MESSAGE submitted to " + toUri + " from " + gsmSender
                + " (SMS id=" + smsId + "); waiting for final SIP status");

        } catch (Exception e) {
            Log.e(TAG, "sendSipMessage FAILED for smsId=" + smsId + ": " + e.getMessage(), e);
            if (pending != null) {
                removePendingSipMessage(pending);
            } else if (buddy != null) {
                deleteBuddy(buddy);
            }
            smsHandler.unprocessSms(smsId);
    ...
    private void handleSipMessageStatus(String to, String body, int code, String reason) {
        if (code < 200) {
            Log.d(TAG, "Ignoring provisional SIP MESSAGE status " + code + " " + reason);
            return;
        }

        PendingSipMessage pending = findPendingSipMessage(to, body);
        if (pending == null) {
            Log.w(TAG, "No pending SMS matches SIP MESSAGE status: to=" + to
                + ", code=" + code + ", body=" + body);
            return;
        }
        ...
        if (code >= 200 && code < 300) {
            smsHandler.markForwarded(pending.smsId);
            boolean markedRead = smsHandler.markAsRead(pending.smsId);
            Log.i(TAG, "SIP MESSAGE confirmed " + code + " for SMS id=" + pending.smsId
                + "; persisted=true, markedRead=" + markedRead);
        } else {
            Log.w(TAG, "SIP MESSAGE failed " + code + " " + reason + " for SMS id=" + pending.smsId);
            retrySms(pending.smsId);
        }
    }

    private PendingSipMessage findPendingSipMessage(String to, String body) {
        synchronized (pendingSipMessages) {
            PendingSipMessage destinationFallback = null;
            for (PendingSipMessage pending : pendingSipMessages) {
                boolean sameDestination = SipUri.extractUser(pending.toUri)
                    .equalsIgnoreCase(SipUri.extractUser(to));
                if (sameDestination && pending.body.equals(body)) {
                    return pending;
                }
                if (sameDestination && destinationFallback == null) {
                    destinationFallback = pending;
                }
            }
    ...
    private void handleSipMessageTimeout(PendingSipMessage pending) {
        synchronized (pendingSipMessages) {
            if (!pendingSipMessages.contains(pending)) {
                return;
            }
        }
        Log.w(TAG, "Timed out waiting for SIP MESSAGE status for SMS id=" + pending.smsId);
        removePendingSipMessage(pending);
        retrySms(pending.smsId);
    }

    private void retrySms(long smsId) {
        smsHandler.unprocessSms(smsId);
        mainHandler.postDelayed(() -> {
            if (isRunning && smsHandler != null) {
                smsHandler.processInbox();
            }
        }, 5_000L);
    }

    private void removePendingSipMessage(PendingSipMessage pending) {
        synchronized (pendingSipMessages) {
            pendingSipMessages.remove(pending);
        }
        deleteBuddy(pending.buddy);
    }

    private static void deleteBuddy(Buddy buddy) {
        try {
            buddy.delete();
        } catch (Exception e) {
            Log.w(TAG, "Failed to delete SIP Buddy: " + e.getMessage());
        }
    }
    ...
    private void checkOrphanedCalls() {
        ...
        if (callManager.isWaitingForDialDigits()) return;
    ...
    public void makeSipCallWithCallerId(String destination, String callerId, int simSlot) {
        try {
            ...
            String displayNumber = callerId == null ? "" : callerId.replaceAll("[^0-9+]", "");
            accountManager.setOutgoingDisplayName(displayNumber);
            ...
            // Keep From tied to the authenticated gateway account, but carry
            // the real GSM caller as the trusted asserted identity.
            if (callerId != null && !callerId.isEmpty()) {
                String accountUri = "sip:" + config.getSipUser() + "@" + server;
                String assertedIdentity = "\"" + displayNumber + "\" <sip:"
                    + displayNumber + "@" + server + ">";

                SipHeader callerIdHeader = new SipHeader();
                callerIdHeader.setHName("X-GSM-CallerID");
                callerIdHeader.setHValue(displayNumber);
                headers.add(callerIdHeader);

                SipHeader preferredIdentityHeader = new SipHeader();
                preferredIdentityHeader.setHName("P-Preferred-Identity");
                preferredIdentityHeader.setHValue(assertedIdentity);
                headers.add(preferredIdentityHeader);

                SipHeader assertedIdentityHeader = new SipHeader();
                assertedIdentityHeader.setHName("P-Asserted-Identity");
                assertedIdentityHeader.setHValue(assertedIdentity);
                headers.add(assertedIdentityHeader);

                SipHeader remotePartyHeader = new SipHeader();
                remotePartyHeader.setHName("Remote-Party-ID");
                remotePartyHeader.setHValue(assertedIdentity
                    + ";party=calling;screen=yes;privacy=off");
                headers.add(remotePartyHeader);

                Log.d(TAG, "Added GSM caller identity headers: " + displayNumber);
            }
