...
public class AudioBridgeManager {
    ...
    private AudioMedia currentCallMedia;
    private ToneGenerator ringbackGenerator;
    private ToneDescVector ringbackTones;
    private ToneDesc ringbackTone;
    private boolean ringbackRequested;
        ...
        try {
            ...
            for (int i = 0; i < mediaVec.size(); i++) {
                ...
                if (mediaInfo.getType() == pjmedia_type.PJMEDIA_TYPE_AUDIO &&
                    ...
                    // PJSUA2 names these from the media port's perspective:
                    // TX is port -> bridge (GSM -> SIP), RX is bridge -> port
                    // (SIP -> GSM).
                    gsmAudioPort.adjustTxLevel(txGain);
                    gsmAudioPort.adjustRxLevel(rxGain);
    ...
    /** Send call-progress audio to SIP while the cellular leg is dialing. */
    public synchronized void startRingback() {
        ringbackRequested = true;
        startRingbackInternal();
    }

    private void startRingbackInternal() {
        if (ringbackGenerator != null || currentCallMedia == null) {
            return;
        }

        try {
            ringbackGenerator = new ToneGenerator();
            ringbackGenerator.createToneGenerator(16000, 1);

            ringbackTone = new ToneDesc();
            ringbackTone.setFreq1((short) 425);
            ringbackTone.setFreq2((short) 0);
            ringbackTone.setOn_msec((short) 1000);
            ringbackTone.setOff_msec((short) 4000);
            ringbackTone.setVolume((short) 8000);
            ringbackTone.setFlags((short) 0);

            ringbackTones = new ToneDescVector();
            ringbackTones.add(ringbackTone);
            ringbackGenerator.play(ringbackTones, true);
            ringbackGenerator.startTransmit(currentCallMedia);
            Log.i(TAG, "SIP ringback started");
        } catch (Exception e) {
            Log.e(TAG, "Failed to start SIP ringback: " + e.getMessage(), e);
            releaseRingbackGenerator();
        }
    }
    ...
    public synchronized void stopRingback() {
        ...
        if (ringbackGenerator == null) {
            return;
        }

        try {
            if (currentCallMedia != null) {
                ringbackGenerator.stopTransmit(currentCallMedia);
            }
            ringbackGenerator.stop();
            Log.i(TAG, "SIP ringback stopped");
        } catch (Exception e) {
            Log.w(TAG, "Failed to stop SIP ringback cleanly: " + e.getMessage());
        } finally {
            releaseRingbackGenerator();
        }
    }

    private void releaseRingbackGenerator() {
        if (ringbackGenerator != null) {
            try {
                ringbackGenerator.delete();
            } catch (Exception ignored) {
            }
        }
        if (ringbackTones != null) {
            ringbackTones.delete();
        }
