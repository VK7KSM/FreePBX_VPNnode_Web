...
public class GatewayInCallService extends InCallService {
    ...
    public void onCallAdded(Call call) {
        ...
        // Telecom commonly delivers an outgoing call already in DIALING state.
        // There may be no separate onStateChanged(DIALING) callback, so feed the
        // initial state to the bridge immediately (notably for SIP ringback).
        PjsipSipService sipService = PjsipSipService.getInstance();
        if (sipService != null) {
            sipService.onGsmCallStateChanged(call, call.getState());
        }
    ...
    /** Keep the dedicated gateway handset silent without muting the PCM bridge. */
    private void silenceLocalAudio() {
        try {
            setAudioRoute(android.telecom.CallAudioState.ROUTE_EARPIECE);
            configureSilentGatewayAudio(this);
        } catch (Exception e) {
            Log.w(TAG, "Unable to silence local call audio: " + e.getMessage());
        }
    }

    /** Apply persistent base volumes before Telecom creates a call. */
    @SuppressWarnings("deprecation")
    public static void configureSilentGatewayAudio(Context context) {
        try {
            ...
            int voiceMinimum = Build.VERSION.SDK_INT >= Build.VERSION_CODES.P
                ? audioManager.getStreamMinVolume(AudioManager.STREAM_VOICE_CALL)
                : 1;
            audioManager.setStreamVolume(AudioManager.STREAM_VOICE_CALL, voiceMinimum, 0);

            // Pixel 3 XL voice-handset uses this amplifier. Disabling only the
            // physical output does not affect VOC_REC_DL or Incall_Music PCM.
            GsmAudioNative.setMixerControl(0, "R AMP Enable Switch", 0);
            Log.d(TAG, "Gateway local audio silenced");
        } catch (Exception e) {
            Log.w(TAG, "Unable to configure silent gateway audio: " + e.getMessage());
        }
    }
}
