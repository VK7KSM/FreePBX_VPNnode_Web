...
public class SmsHandler {
    ...
    private static final String PREFS_FORWARD_STATE = "sms_forward_state";
    private static final String KEY_FORWARDED_IDS = "forwarded_ids";
    private static final int MAX_FORWARDED_IDS = 256;
    ...
    private final Set<Long> forwardedSmsIds = new HashSet<>();
    ...
    private void loadForwardedIds() {
        Set<String> stored = context.getSharedPreferences(PREFS_FORWARD_STATE, Context.MODE_PRIVATE)
            .getStringSet(KEY_FORWARDED_IDS, java.util.Collections.emptySet());
        for (String value : stored) {
            try {
                processedSmsIds.add(Long.parseLong(value));
                forwardedSmsIds.add(Long.parseLong(value));
            } catch (NumberFormatException e) {
                Log.w(TAG, "Ignoring invalid persisted SMS ID: " + value);
            }
        }
        Log.d(TAG, "Loaded forwarded SMS IDs: " + processedSmsIds);
    }

    public synchronized void markForwarded(long smsId) {
        processedSmsIds.add(smsId);
        forwardedSmsIds.add(smsId);
        persistForwardedIds(context, forwardedSmsIds);
    }

    public static void rememberForwardedSms(Context context, long smsId) {
        SharedPreferences prefs = context.getSharedPreferences(PREFS_FORWARD_STATE, Context.MODE_PRIVATE);
        Set<String> values = new LinkedHashSet<>(
            prefs.getStringSet(KEY_FORWARDED_IDS, java.util.Collections.emptySet()));
        values.add(Long.toString(smsId));
        trimForwardedIds(values);
        prefs.edit().putStringSet(KEY_FORWARDED_IDS, values).commit();
    }

    private static void persistForwardedIds(Context context, Set<Long> ids) {
        Set<String> values = new LinkedHashSet<>();
        for (Long id : ids) {
            values.add(Long.toString(id));
        }
        trimForwardedIds(values);
        context.getSharedPreferences(PREFS_FORWARD_STATE, Context.MODE_PRIVATE)
            .edit().putStringSet(KEY_FORWARDED_IDS, values).commit();
    }

    private static void trimForwardedIds(Set<String> values) {
        while (values.size() > MAX_FORWARDED_IDS) {
    ...
    public boolean markAsRead(long smsId) {
        try {
            ...
            if (updated > 0) {
                ...
                Log.w(TAG, "Failed to mark SMS id=" + smsId + " as read; persistent forwarding state will suppress duplicates");
            }
        } catch (Exception e) {
