...
final class GatewayPixelRuntimeHealth {
    ...
    private static String read(File file,int maximum)throws Exception {if(!file.isFile())throw new IOException("health source unavailable");try(InputStream input=new FileInputStream(file);ByteArrayOutputStream output=new ByteArrayOutputStream()){byte[] buffer=new byte[Math.min(64,maximum+1)];for(int count;(count=input.read(buffer))!=-1;){output.write(buffer,0,count);if(output.size()>maximum)throw new IOException("health source too large");}return output.toString("US-ASCII");}}
