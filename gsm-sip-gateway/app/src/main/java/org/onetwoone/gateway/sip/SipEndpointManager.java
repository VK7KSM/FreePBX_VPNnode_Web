...
public class SipEndpointManager {
    ...
    public SipEndpointManager(Context context, GatewayConfig config) {
    ...
    private void createEndpointInternal(boolean useTls) throws Exception {
        ...
        if (config.isLinphoneService()) {
            uaConfig.getStunServer().add("stun.linphone.org");
        ...
        mediaConfig.setAudioFramePtime(20);
        mediaConfig.setJbInit(0);
        mediaConfig.setJbMinPre(0);
        mediaConfig.setJbMaxPre(40);
        mediaConfig.setJbMax(80);
    ...
    private void createTransport(boolean useTls) throws Exception {
        ...
        if (useTls) {
            ...
            // Verify the PBX against the bundled Mozilla CA trust store.
            TlsConfig tlsConfig = transportConfig.getTlsConfig();
            tlsConfig.setCaBuf(loadCaBundle());
            tlsConfig.setVerifyServer(true);
            tlsConfig.setVerifyClient(false);
            tlsConfig.setRequireClientCert(false);
