...
public class GatewayConfig {
    ...
    public static final String AUDIO_ROUTE_UNCONFIGURED = "UNCONFIGURED";
    public static final String PIXEL_CAPTURE_ROUTE = "MultiMedia1";
    public static final String PIXEL_INJECTION_ROUTE = "MultiMedia9";
    public static final int PIXEL_CAPTURE_DEVICE = 0;
    public static final int PIXEL_PLAYBACK_DEVICE = 27;
    private static final String DEFAULT_MUTE_PRESET = "pixel_3_xl";
    ...
    private static final String KEY_CAPTURE_ROUTE = "capture_route";
    private static final String KEY_INJECTION_ROUTE = "injection_route";
    private static final String KEY_MULTIMEDIA_ROUTE = "multimedia_route"; // Legacy single-route setting
    ...
    private static final int DEFAULT_CAPTURE_DEVICE = PIXEL_CAPTURE_DEVICE;
    private static final int DEFAULT_PLAYBACK_DEVICE = PIXEL_PLAYBACK_DEVICE;
    ...
    public boolean isLinphoneService() {
        return "sip.linphone.org".equalsIgnoreCase(getSipServer());
    }

    /** Public SIP peers must be explicitly configured before they may use the SIM. */
    public boolean isAuthorizedSipCaller(String callerExt) {
        if (callerExt == null || callerExt.isEmpty()) return false;
        return callerExt.equals(getSim1Destination()) ||
            (!getSim2Destination().isEmpty() && callerExt.equals(getSim2Destination()));
    }
    ...
    public String getCaptureRoute() {
        if (audioPrefs.contains(KEY_CAPTURE_ROUTE)) {
            return audioPrefs.getString(KEY_CAPTURE_ROUTE, PIXEL_CAPTURE_ROUTE);
        }
        return audioPrefs.getString(KEY_MULTIMEDIA_ROUTE, PIXEL_CAPTURE_ROUTE);
    }

    public void setCaptureRoute(String route) {
        audioPrefs.edit().putString(KEY_CAPTURE_ROUTE, route).apply();
    }

    public String getInjectionRoute() {
        if (audioPrefs.contains(KEY_INJECTION_ROUTE)) {
            return audioPrefs.getString(KEY_INJECTION_ROUTE, PIXEL_INJECTION_ROUTE);
        }
        return audioPrefs.getString(KEY_MULTIMEDIA_ROUTE, PIXEL_INJECTION_ROUTE);
    }

    public void setInjectionRoute(String route) {
        audioPrefs.edit().putString(KEY_INJECTION_ROUTE, route).apply();
    }

    /** Compatibility for callers that still supply one route for both directions. */
    @Deprecated
    public String getMultimediaRoute() {
        return getCaptureRoute();
    }
    ...
    public void setMultimediaRoute(String route) {
        audioPrefs.edit()
            .putString(KEY_CAPTURE_ROUTE, route)
            .putString(KEY_INJECTION_ROUTE, route)
     ...
     */
    public void updateAudioConfig(int card, int capture, int playback,
        ...
        audioPrefs.edit()
            ...
            .putString(KEY_CAPTURE_ROUTE, captureRoute)
            .putString(KEY_INJECTION_ROUTE, injectionRoute)
            .remove(KEY_MULTIMEDIA_ROUTE)
            .apply();
    }
    ...
    public void updateAudioConfig(int card, int capture, int playback, String route) {
        updateAudioConfig(card, capture, playback, route, route);
    }
