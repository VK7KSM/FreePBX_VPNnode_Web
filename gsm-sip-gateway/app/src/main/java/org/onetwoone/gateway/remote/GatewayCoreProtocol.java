...
final class GatewayCoreProtocol {
    ...
    static boolean allowed(JSONObject request, String expected) {
        ...
        return expected != null && expected.matches("[0-9a-f]{64}")
                ...
                && java.util.Arrays.asList("health","pixel-module-health","pixel-runtime-health","mobile-status","proxy-prepare","proxy-configure","proxy-start","proxy-stop","proxy-test","proxy-status","desktop-prepare","desktop-start","desktop-stop","desktop-status","push-config","push-status","push-tick","push-ack","push-hint","shutdown")
