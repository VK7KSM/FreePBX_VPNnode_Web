...
final class GatewayCoreProtocol {
    ...
    static boolean allowed(JSONObject request, String expected) {
        ...
        return expected != null && expected.matches("[0-9a-f]{64}")
                ...
                && java.util.Arrays.asList("health","pixel-module-health","mobile-status","proxy-prepare","proxy-configure","proxy-start","proxy-stop","proxy-test","proxy-status","push-config","push-status","push-tick","push-ack","push-hint","shutdown")
