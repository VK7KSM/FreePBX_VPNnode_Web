...
function renderRemoteConsole(){
  ...
  var h='<div class="remote-head"><div class="remote-title"><h3>通信终端</h3>'+(media?.connectionControl(d)||'')+'<span id="serviceError" role="status"'+(errorText?'':' hidden')+'>'+esc(errorText)+'</span></div><div class="remote-head-actions"><span class="remote-device">'+esc(d?d.name:'未选择设备')+'</span><button type="button" class="traffic-link" onclick="openMediaHistory()"'+(d?'':' disabled')+'>历史记录</button></div></div>';
