...
function fixture(mode='call'){
 ...
 class Peer{constructor(){peers.push(this);this.connectionState='new';this.transceivers=[];}addTransceiver(t){this.transceivers.push({mid:'0',sender:{track:t}});}getTransceivers(){return this.transceivers;}async createOffer(){return {type:'offer',sdp:'local'};}async createAnswer(){return {type:'answer',sdp:'answer'};}async setLocalDescription(d){this.localDescription={...d,toJSON:()=>({...d})};}async setRemoteDescription(d){this.remoteDescription=d;}close(){this.connectionState='closed';}connect(){this.connectionState='connected';this.onconnectionstatechange?.();}remote(t=track()){this.ontrack?.({track:t});return t;}}
